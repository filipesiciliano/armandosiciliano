...
document.querySelectorAll('.scroll-reveal, .animate-slide-up, .animate-fade-in').forEach(el => {
    observer.observe(el);
});
...
window.addEventListener('scroll', () => {
    ...
    if (scrollY > heroBottom && scrollY < checkoutTop - 200) {
...
// Track button clicks
document.querySelectorAll('button[onclick*="scrollToCheckout"]').forEach(btn => {
...
document.addEventListener('click', (e) => {
    if (e.target.textContent.includes('COMPRAR') || e.target.textContent.includes('Continuar')) {
